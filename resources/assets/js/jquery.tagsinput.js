...
(function($) {
  ...
  $.fn.resetAutosize = function(options){
	...
	$.fn.addTag = function(value,options) {
			...
			this.each(function() { 
				...
				if (value !='' && skipTag != true) {
                    ...
                    if (index == 0 || index == -1) {
                    	element.insertAfter('#'+id+'_addTag');
                    } else {
                    	var insert_after = tags[--index]; 
                    	element.insertAfter('#'+id+'_tagsinput .tag[data-tag='+insert_after+']');
                    }
	...
	$.fn.getTags = function() {
		var tags = [];
		$('#'+$(this).attr('id')+'_tagsinput .tag').each(function(el) {
	...
	$.fn.removeTag = function(value) {
			...
			this.each(function() {
				...
				$('#'+id+'_tagsinput .tag').each(function(el) {
					if ($(this).attr('data-tag') === value) {
						$(this).remove();
					}
				});

				$(this).val($(this).getTags().join(','));
	...
	$.fn.tagsInput.importTags = function(obj,val) {			
		...
		for (i=0; i<tags.length; i++) { 
			$(obj).addTag(tags[i],{focus:false,callback:false, animate: false});
		}
		if(tags_callbacks[id] && tags_callbacks[id]['onChange'])
		{
			var f = tags_callbacks[id]['onChange'];
