    ...
    <script>
        kQuery(function($)
        {
            Attachments = Attachments.getInstance(
                {
                    url: "<?= route('view=files&routed=1&name={name}&format={format}&thumbnails=1', true, false) ?>",
            ...
            var attachments = <?= json_encode(helper('com:files.attachments.fileinfo', array('attachments' => $attachments))) ?>
...
<? if (isset($select) && $select == true): ?>
    <script>
        kQuery(function($)
        {
            ...
            {
                $.ajax({
                    url: Attachments.route(selected, 'json'),
                    success: function(data) {
                        Attachments.insert(data.entities.pop());
                    }
                });
